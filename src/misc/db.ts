...
class Database {
    ...
    constructor() {
        this.db = new sql('database.db', { verbose: console.debug });
    ...
    addUser(discordId: string, pteroUserId: string, email: string, username: string) {
        this.db.prepare('INSERT INTO ptero_users (discord_id, ptero_user_id, email, username) VALUES (?, ?, ?, ?)')
            .run(discordId, pteroUserId, email, username);
    }

    updateUser(discordId: string, pteroUserId: string, email: string, username: string) {
        this.db.prepare('UPDATE ptero_users SET ptero_user_id = ?, email = ?, username = ? WHERE discord_id = ?')
    ...
    addServer(discordId: string, pteroServerId: string) {
        this.db.prepare('INSERT INTO ptero_servers (discord_id, ptero_server_id) VALUES (?, ?)')
            .run(discordId, pteroServerId);
    }

    updateServer(discordId: string, pteroServerId: string) {
        this.db.prepare('UPDATE ptero_servers SET ptero_server_id = ? WHERE discord_id = ?')
